...
lndmrk_poses = np.array([[0.75, 0.75], [-0.75, -0.75], [0.75, -0.75]])
default_agent_poses = [[0.5, 0.5], [-0.5, -0.5], [0.5, -0.5]]
flipped_order = [1,2,0]
...
def switch_action(onehot):
    output = onehot.data.numpy()
    output[0][2], output[0][1] = output[0][1], output[0][2]
    output[0][4], output[0][3] = output[0][3], output[0][4]
    return torch.Tensor(output)

def heatmap3(maddpg, title="Agent Policies", save=False):
