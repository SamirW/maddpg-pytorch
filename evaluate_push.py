...
def run(config):
    #hard code
    combine = False
    if config.model_name == 'mode0':
        config.run_num = 2
        model_ep = 9001
    elif config.model_name == 'mode1':
        config.run_num = 1
        model_ep = 19501
    elif config.model_name == 'mode2':
        combine = True
        config.run_num = 2
        run_dir = Path('./models') / 'complex_push' / 'mode2' / "run{}".format(config.run_num)
        run_dir1 = Path('./models') / 'complex_push' / 'mode0' / "run2"
        run_dir2 = Path('./models') / 'complex_push' / 'mode1' / "run1"

        model_file1 = run_dir1 / 'incremental' / 'model_ep9001.pt'
        model_file2 = run_dir2 / 'incremental' / 'model_ep19501.pt'

    if not combine:
        model_dir = Path('./models') / config.env_id / config.model_name
        run_dir = model_dir / "run{}".format(config.run_num)
        model_file = run_dir / "incremental" / "model_ep{}.pt".format(model_ep)
    ...
    if not combine:
        maddpg = MADDPGPUSH.init_from_save(str(model_file))
    else:
